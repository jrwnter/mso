...
_default_desirability = [{"x": 0.0, "y": 0.0}, {"x": 1.0, "y": 1.0}]
...
class ScoringFunction:
    ...
    def __init__(self, func, name, description=None, desirability=None, truncate_left=True,
        ...
        :param desirability: A list of dictionaries where each dictionary {"x": x, "y": y} defines a point on the
            desirability curve used to scale the output of the scoring function into the range [0, 1]. If None, a
            default desirability curve is used which is linear in teh range [0, 1].
        :param truncate_left: Flag whether the desirability is truncated on the left side (lowest defined x), thus set
            to the same const value for all smaller x or linearly extrapolated.
        :param truncate_right: Flag whether the desirability is truncated on the right side (highest defined x), thus
            set to the same const value for all higher x or linearly extrapolated.
        :param weight: The weight of the scoring function in the combined (weighted average) score in a multi-objective
            optimization.
        :param is_mol_func: Flag that defines if the scoring function expects a RDKit mol object (True) or an array of
    ...
    def _create_desirability_function(self, desirability, truncate_left=True, truncate_right=True):
        """
        Method that returns a function that calculates the desirability score for a given input unscaled score. Linearly
            interpolates between points provided.
        :param desirability: List of dictionaries that define points that lie on the desirability curve.
        :param truncate_left: Flag whether the desirability is truncated on the left side (lowest defined x), thus set
            to the same const value for all smaller x or linearly extrapolated.
        :param truncate_right: Flag whether the desirability is truncated on the right side (highest defined x), thus
            set to the same const value for all higher x or linearly extrapolated.
        :return: A function that calculates the desirability score for a given input unscaled score.
        """
    ...
    def __call__(self, input):
        """
        Calling a ScoringFunction instance evaluates the scoring function and rescales the scores with respect to the
        desirability scaling and the weight.
        :param input: Either a RDKit mol object or an array of particle positions (num_particles, ndim) in the CDDD
            space.
        :return:
            unscaled_scores: The unscaled output of the scoring function call.
            scaled_scores: The unscaled score scaled with respect to the desirability curve and multiplied by the
    ...
    def __repr__(self):
        return 'cddd.optimization.scoring.ScoringFunction name={} desirability={} weight={}'.format(self.name,
                                                                                                    self._desirability,
                                                                                                    self.weight)
